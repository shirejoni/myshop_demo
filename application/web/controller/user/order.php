...
class ControllerUserOrder extends Controller {
    ...
    public function index(): void
    {
        ...
        $this->Response->setOutPut($this->render('user/order/index', $data));
    }
    ...
    public function edit()
    {
        ...
        if(isset($this->Request->get[0])) {
            ...
            if($order_id && (int) $order_id && $order['order_status_id'] >= 2) {
                ...
                $this->Response->setOutPut($this->render('user/order/edit', $data));
                return;
            }
        }
        ...
        if(isset($this->Request->get[0])) {
            ...
            if($order_id && (int) $order_id && $order['order_status_id'] >= 2) {
                ...
                if($this->Customer->getCustomerId() == $customer_id) {
                    $Order->editOrder($order_id, array(
                        'order_status_id'   => 7
                    ));
                    $Order->insertOrderHistory(array(
                        'order_id'  => $order_id,
                        'order_status_id'   => 7,
                        'date_added'        => time()
                    ));
                    ...
                    $json['redirect'] = URL . 'user/order/index';
